...
def _load_dataset(
        ...
        shuffle = False) -> Tuple[torch.Tensor, torch.Tensor]:
...
def load_cifar10(
    ...
    dataset = datasets.CIFAR10(root=data_dir,
                               ...
                               download=True)
    return _load_dataset(dataset, n_examples, shuffle=shuffle)
...
def load_svhn(
    n_examples: Optional[int] = 10000, # this is n_cifar_samples
    data_dir: str = './data', 
    #batch_size: int = None, 
    transforms_test: Callable = PREPROCESSINGS[None],
    shuffle = False
    ...
    #dataloader = data.DataLoader(dataset, batch_size=batch_size, num_workers=8, shuffle=True)
...
def load_svhnc(
        ...
        corruptions: Sequence[str] = CORRUPTIONS,
        _: Callable = PREPROCESSINGS[None]) -> Tuple[torch.Tensor, torch.Tensor]:
    return load_corruptions_svhn(n_examples,
                                  severity, data_dir, corruptions, shuffle)
