...
def frost(x, severity=1):
    ...
    data_dir: str = '06_new_c_datasets/'
    filename = ['frost1.png', 'frost2.png', 'frost3.png', 'frost4.jpg', 'frost5.jpg', 'frost6.jpg'][idx]
    path = data_dir + filename
    #print(path, imghdr.what(path))
...
def create_corrupted_dataset(dataset, data_dir):

    print('Using {} data'.format(dataset))
    #print('Using SVHN data')
    ...
    if dataset == "SVHN":
        test_data = dset.SVHN(root = data_dir, split="test", download=True)
    elif dataset == "CIFAR-10":
         test_data = dset.CIFAR10(root = data_dir, train=False, download=True)
    ...
    for method_name in d.keys():
        ...
        for severity in range(1,6):
            ...
            #for img, label in zip(test_data.data, test_data.labels):
                ...
                for img, label in zip(test_data.data, test_data.targets):
                    #print(label, img.shape) # cifar: 3 (32, 32, 3), svhn: 5 (3, 32, 32)
                    labels.append(label)
                    cifar_c.append(np.uint8(corruption(convert_img(img))))


        #np.save(data_dir + 'CIFAR-10-C_2/' + d[method_name].__name__ + '.npy',
        np.save(data_dir + dataset + '-C/' + d[method_name].__name__ + '.npy',
                np.array(cifar_c).astype(np.uint8))

        #np.save(data_dir + 'CIFAR-10-C_2/labels.npy',
        np.save(data_dir + dataset + '-C/labels.npy',
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Create corrupted dataset')
    parser.add_argument('--ds', dest='dataset', type=str, default="CIFAR-10", help='the dataset to download+corrupt')
    parser.add_argument('--dd', dest='data_dir', type=str, default='06_new_c_datasets/', help='the directory in which the files will be saved. Should contain frost jpgs/pngs')
    args = parser.parse_args()

    assert args.dataset in ["SVHN", "CIFAR-10"], "Function not available for this dataset. received {}. Should be in ['SVHN', 'CIFAR-10']".format(args.dataset)

    create_corrupted_dataset(dataset=args.dataset, data_dir=args.data_dir)
