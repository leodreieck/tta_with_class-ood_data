...
# threshold for hard pl + gce
_C.MODEL.PL_THRESHOLD = 0.

_C.MODEL.CREATE_EMBEDDINGS = False

_C.MODEL.OOD_METHOD = "none" # takes [”no”, “threshold_n_images”, “threshold_confidence”]

_C.MODEL.OOD_THRESHOLD = 0.
...
_C.CORRUPTION.SVHN_samples = 0

_C.CORRUPTION.CIFAR100_samples = 0

_C.CORRUPTION.SVHNC_samples = 0

_C.CORRUPTION.CIFAR100C_samples = 0
...
def load_cfg_fom_args(description="Config options."):
    ...
    if cfg.CORRUPTION.SVHN_samples > 0:
        ood_logstr = "SVHN_{}".format(cfg.CORRUPTION.SVHN_samples)
    elif cfg.CORRUPTION.SVHNC_samples > 0:
        ood_logstr = "SVHNC_{}".format(cfg.CORRUPTION.SVHNC_samples)
    elif cfg.CORRUPTION.CIFAR100_samples > 0:
        ood_logstr = "CIFAR100_{}".format(cfg.CORRUPTION.CIFAR100_samples)
    elif cfg.CORRUPTION.CIFAR100C_samples > 0:
        ood_logstr = "CIFAR100C_{}".format(cfg.CORRUPTION.CIFAR100C_samples)
    else:
        ood_logstr = "NO-OOD"
    log_dest = log_dest.replace('.yaml', '_{}_{}_{}.txt'.format(ood_logstr, current_time, cfg.CORRUPTION.TYPE[0][:4]))

    g_pathmgr.mkdirs(cfg.SAVE_DIR)
    cfg.LOG_TIME, cfg.LOG_DEST = current_time, log_dest
    cfg.freeze()

    assert cfg.MODEL.OOD_METHOD in ["none", "threshold_n_images", "threshold_confidence"], "chosen OOD method not implemented"
    if cfg.MODEL.OOD_METHOD in ["threshold_n_images", "threshold_confidence"]:
        assert cfg.MODEL.OOD_THRESHOLD not in [0.0, 1.0], "Active ood detection method, but useless threshold selected"
        #assert ood_logstr != "NO-OOD", "Cannot run ood detection without ood samples"
